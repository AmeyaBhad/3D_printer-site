...
public class RuleBasedChatService implements ChatService {
    ...
    public ChatResponse reply(String userMessage) {
        ...
        if (msg.isEmpty()) {
            return msg("Tell me what you're looking for — products, prices, shipping, or anything else about Printed Dimensions.");
        }

        // Greetings
        if (containsAny(msg, "hi", "hello", "hey", "good morning", "good afternoon", "good evening", "namaste")) {
            return msg("Hi there! 👋 I'm Dimi, your Printed Dimensions assistant. What can I help you with?", DEFAULT_SUGGESTIONS);
        }
        ...
        if (containsAny(msg, "custom", "made to order", "design my own", "personalized", "bespoke", "commission")) {
            return msg("Yes! We do custom 3D prints. Send your idea (sketches, reference images, or an STL file) to contact@printeddimensions.com or via the Contact page and we'll get back with a quote.");
        }

        // Returns
        if (containsAny(msg, "return", "refund", "exchange")) {
            return msg("Standard items can be returned within 7 days if unused and in original packaging. Custom and made-to-order pieces are non-refundable. Email contact@printeddimensions.com to start a return.");
        }
